...
class BusWebObserver : public IObserver<BusData*> {
 public:
    ...
    void UpdateObserver(BusData* info) override {
        // This normally called update, but we call
        // it Notify as per the lab writeup
        picojson::object data;
...
AddListenerCommand::AddListenerCommand
(VisualizationSimulator* sim) : mySim(sim) {}
...
// add stop listener
AddListenerCommandStop::AddListenerCommandStop
(VisualizationSimulator* sim) : mySim(sim) {}

void AddListenerCommandStop::execute(MyWebServerSession* session,
picojson::value& command, MyWebServerSessionState* state) {
    mySim->ClearListenersStop();
...
InitRoutesCommand::InitRoutesCommand
(ConfigManager* configManager) : cm(configManager) {}
